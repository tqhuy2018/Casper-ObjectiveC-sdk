...
-(NSString*) toJsonStringWithMethodName:(NSString*) methodName {
    ...
    if(self.blockType == USE_BLOCK_HASH) {
        ...
        retJsonStr = [[NSString alloc] initWithFormat:@"{\"method\" : \"%@\",\"id\" : 1,\"params\" : {\"block_identifier\" : {\"Height\" :%d}},\"jsonrpc\" : \"2.0\"}",methodName,self.blockHeight];
    } else {
        retJsonStr = [[NSString alloc] initWithFormat:@"{\"method\" : \"%@\",\"id\" : 1,\"params\" :\"[]\",\"jsonrpc\" : \"2.0\"}",methodName,self.blockHeight];
    }
    NSLog(@"retJsonString:%@",retJsonStr);
    return retJsonStr;
}
@end
